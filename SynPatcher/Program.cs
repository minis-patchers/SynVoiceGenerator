...
public static class Program
{
    ...
    static bool IsValidChar(char x) => Char.IsAsciiLetterOrDigit(x) || x == '<' || x == '>' || x == ' ' || x == '.' || x == ',' || x == '?' || x == '!' || x == '\"' || x == '\'' || x == '*' || x == '[' || x == ']' || x == '(' || x == ')';
    static string CleanString(string str) => new string(REG.HiddenFN2.Replace(REG.HiddenFN.Replace(str, ""), "").Replace("...", "").Where(IsValidChar).ToArray()).Trim();
    ...
    static LineData? GenerateLive(string text)
    {
        ...
        if (!File.Exists(fuzname) && File.Exists(wavname))
        {
            ...
            {
                var mpeg = Process.Start($"{EDFP}/ffmpeg.exe", $"-i \"{wavname}\" -ac 1 \"{mp3name}\"");
                mpeg.WaitForExit();
                if (mpeg.ExitCode != 0) return null;
            }
            LineData ret = new()
            {
            ...
            var p = Process.Start($"{EDFP}/FaceFXWrapper.exe", $"Skyrim USEnglish FonixData.cdf \"{wavname}\" \"{rwavnam}\" \"{lipname}\" \"{text.Replace("\"", "\\\"")}\"");
            p.WaitForExit();
